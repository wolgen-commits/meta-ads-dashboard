...
function MediaCard({ media }: { media: any }) {
  const eng = (media.likes ?? 0) + (media.comments ?? 0) + (media.shares ?? 0) + (media.saved ?? 0);
...
function AccountTab({ accountId, accountName }: { accountId: string; accountName: string }) {
  ...
  return (
    <div className="ig-account-tab">
      ...
      <section className="kpi-grid">
        <KpiCard label="Total Likes"     value={summary ? num(summary.total_likes) : "—"}    loading={loadingSummary} description="Total likes dari semua post yang tersimpan." />
        ...
        <KpiCard label="Total Reach"     value={summary ? num(summary.total_reach) : "—"}    loading={loadingSummary} description="Total jangkauan dari semua post." />
        <KpiCard
          label="Avg Engagement Rate"
          value={summary ? `${summary.avg_engagement_rate.toFixed(2)}%` : "—"}
          loading={loadingSummary}
          description="Rata-rata engagement rate dihitung dari (likes+comments+shares+saves) / reach × 100."
        />
      </section>

      {/* Media grid */}
      <div className="media-section">
        <h3 className="chart-title">Post terbaru</h3>
        {loadingMedia ? (
          ...
          <div className="media-grid">
            ...
            {(media ?? []).length === 0 && <p style={{ opacity: 0.5 }}>Belum ada data media.</p>}
          </div>
        )}
      </div>
    </div>
...
export function InstagramTab() {
  ...
  if (!accounts || accounts.length === 0) return <p style={{ opacity: 0.5 }}>Belum ada akun Instagram yang terhubung.</p>;
    ...
    <div className="ig-tab">
      ...
      <div className="ig-account-switcher">
        {accounts.map((a) => (
          ...
          >
            <span className="ig-at">@</span>{a.username ?? a.name}
      ...
      {currentAccount && (
        <AccountTab accountId={currentAccount.id} accountName={currentAccount.name} />
      )}
    </div>
  );
}
