...
export function DemographicChart({ dateStart, dateStop, campaignIds = [] }: Props) {
  ...
  const byAge = (data ?? []).reduce<Record<string, { age: string; perempuan: number; laki: number; lainnya: number }>>((acc, row) => {
    ...
    if (gender === "female")       acc[age].perempuan += row.impressions ?? 0;
    else if (gender === "male")    acc[age].laki      += row.impressions ?? 0;
    else                           acc[age].lainnya   += row.impressions ?? 0;
    return acc;
  }, {});
  ...
  if (chartData.length === 0) return (
    <div className="chart-card">
      <h3 className="chart-title">Demografis Usia & Gender</h3>
